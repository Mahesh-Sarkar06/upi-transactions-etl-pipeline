...
def getPGUsers():
    ...
    try:
        ...
        print(f"Failed to connect with Database: {err}")
        return []
    finally:
        if conn:
            conn.close()
...
def producerMain():
    parser = argparse.ArgumentParser(description='UPI Kafka Continuous data')
    parser.add_argument('--kafka-bootstrap', required=True, help='Kafka server URL')
    parser.add_argument('--kafka-topic', required=True, help='Kafka topic to queue messages')
    parser.add_argument('--users', required=True, type=int, help='Total number of users to be fetched')
    parser.add_argument('--group-id', default='UPI-Databricks', help='Group where messages to be delivered')
    parser.add_argument('--count', default=500, type=int, help='Number of records to be generated')
    parser.add_argument('--failed-pct', required=True, type=float, help='Total error records')
    parser.add_argument('--rate', type=float, default=0.0, help='Number of records per second')
    ...
    print(f"Generating continuous user transaction data to topic: {topic}.")
    ...
    try:
        ...
        print("Keyboard interruption found (Ctrl+C). Flushing all the remaining messages...")
